  ...
  return (
    <div className="relative w-full aspect-video md:aspect-[16/9] bg-slate-950 rounded-2xl overflow-hidden border-2 border-slate-800 shadow-2xl flex flex-col select-none">
      ...
      <div className="absolute top-2 right-2 sm:top-4 sm:right-4 z-10 flex flex-col items-end gap-1.5 sm:gap-2">
        <div className="bg-slate-900/95 backdrop-blur-md p-1 sm:p-2 rounded-xl border border-slate-700/60 shadow-2xl">
          <canvas
            ref={minimapRef}
            width={128}
            height={128}
            className="rounded border border-slate-800 shadow-inner w-[70px] h-[70px] sm:w-[128px] sm:h-[128px]"
            style={{ imageRendering: 'pixelated' }}
          />
          <div className="hidden sm:flex text-[10px] text-center text-slate-400 mt-1.5 font-mono items-center justify-center gap-1">
        ...
        <div className="flex gap-1 sm:gap-2">
          <button
            ...
            className={`px-1.5 py-1 sm:px-2.5 sm:py-2 rounded-lg border transition-all shadow-md cursor-pointer flex items-center gap-1 font-mono text-[9px] sm:text-xs ${
              showTouchControls 
          ...
          <button
            ...
            className="px-1.5 py-1 sm:px-2.5 sm:py-2 bg-slate-900/90 hover:bg-slate-800/90 rounded-lg border border-slate-700/60 text-slate-300 transition-all shadow-md cursor-pointer flex items-center gap-1 font-mono text-[9px] sm:text-xs"
            title="Ver Inventario"
          >
            <Backpack className="w-3 h-3 sm:w-4 sm:h-4 text-amber-500 animate-pulse" />
            <span className="hidden xs:inline">Inv [I]</span>
            <span className="xs:hidden">Inv</span>
          </button>
          <button
            onClick={() => setSoundEnabled(!soundEnabled)}
            className="p-1 sm:p-2 bg-slate-900/90 hover:bg-slate-800/90 rounded-lg border border-slate-700/60 text-slate-300 transition-all shadow-md cursor-pointer"
            title="Toggle SFX"
          >
            {soundEnabled ? <Volume2 className="w-3 h-3 sm:w-4 sm:h-4 text-amber-500" /> : <VolumeX className="w-3 h-3 sm:w-4 sm:h-4 text-slate-500" />}
          </button>
        </div>
      </div>

      {/* Floating Status / Quest Log: placed below StatsOverlay on mobile, bottom-left on desktop */}
      <div className="absolute top-[102px] left-2 sm:top-auto sm:bottom-4 sm:left-4 z-10 max-w-[150px] sm:max-w-sm bg-slate-900/90 backdrop-blur-sm p-1 sm:px-3.5 sm:py-1.5 rounded-lg border border-slate-800 text-[8px] sm:text-[11px] font-mono text-emerald-400 flex items-center gap-1 sm:gap-2 shadow-lg">
        <Sparkles className="w-2.5 h-2.5 sm:w-3.5 sm:h-3.5 text-amber-400 flex-shrink-0 animate-bounce" />
        <span className="truncate">{recentLog}</span>
      </div>

      {/* Inventory Modal Overlay */}
      {showInventory && (
        <div className="absolute inset-0 z-40 bg-slate-950/85 backdrop-blur-sm flex items-center justify-center p-2 sm:p-6 select-none">
          <div className="bg-slate-900 border-2 border-slate-700 rounded-2xl w-full max-w-2xl h-[92%] sm:h-[85%] flex flex-col shadow-2xl overflow-hidden font-sans">
            {/* Header */}
            <div className="px-3 py-2.5 sm:px-5 sm:py-3.5 bg-slate-800/50 border-b border-slate-700/60 flex items-center justify-between">
              <div className="flex items-center gap-2 sm:gap-2.5">
                <div className="p-1 sm:p-1.5 bg-amber-500/10 rounded-lg border border-amber-500/20">
                  <Backpack className="w-4 h-4 sm:w-5 sm:h-5 text-amber-500" />
                </div>
                <div>
                  <h3 className="font-semibold text-slate-100 tracking-tight text-xs sm:text-base">Mochila del Héroe</h3>
                  <p className="text-[8px] sm:text-[10px] text-slate-400 font-mono">Espacio ocupado: {playerState.inventory?.reduce((acc, item) => acc + item.quantity, 0) || 0} objetos</p>
                </div>
              </div>
              <button
            ...
            <div className="flex-1 overflow-hidden flex flex-col md:flex-row divide-y md:divide-y-0 md:divide-x divide-slate-800">
              ...
              <div className="flex-1 overflow-y-auto p-2 sm:p-4 space-y-1.5 sm:space-y-2">
                {(!playerState.inventory || playerState.inventory.length === 0) ? (
                  <div className="h-full flex flex-col items-center justify-center text-center p-4 sm:p-6 space-y-2 sm:space-y-3">
                    <div className="p-2 sm:p-3 bg-slate-800/40 rounded-full text-slate-500">
                      <Backpack className="w-8 h-8 sm:w-10 sm:h-10 stroke-[1.5]" />
                    </div>
                    <div className="space-y-0.5 sm:space-y-1">
                      <p className="text-xs sm:text-sm font-medium text-slate-300">Tu mochila está vacía</p>
                      <p className="text-[10px] sm:text-xs text-slate-500 max-w-[200px] mx-auto">Busca objetos brillantes con estrellas parpadeantes en el mapa.</p>
                    </div>
                  </div>
                ) : (
                  playerState.inventory.map((item, idx) => {
                    ...
                    return (
                      <button
                        ...
                        className={`w-full text-left p-1.5 sm:p-2.5 rounded-xl border flex items-center justify-between transition-all cursor-pointer ${
                          isSelected
                      ...
                      >
                        <div className="flex items-center gap-2 sm:gap-3">
                          <div
                            className="w-6 h-6 sm:w-8 sm:h-8 rounded-lg flex items-center justify-center border"
                            style={{
                              backgroundColor: `${item.spriteColor}15`,
                              borderColor: `${item.spriteColor}30`,
                            }}
                          >
                            {item.type === 'weapon' && <Sword className="w-3.5 h-3.5 sm:w-4 sm:h-4" style={{ color: item.spriteColor }} />}
                            {item.type === 'shield' && <Shield className="w-3.5 h-3.5 sm:w-4 sm:h-4" style={{ color: item.spriteColor }} />}
                            {item.type === 'potion' && <Heart className="w-3.5 h-3.5 sm:w-4 sm:h-4" style={{ color: item.spriteColor }} />}
                            {item.type === 'key_item' && (
                              item.name === 'Gema Estelar'
                                ? <Star className="w-3.5 h-3.5 sm:w-4 sm:h-4" style={{ color: item.spriteColor }} />
                                : <Key className="w-3.5 h-3.5 sm:w-4 sm:h-4" style={{ color: item.spriteColor }} />
                            )}
                            {item.type === 'material' && <Sparkles className="w-3.5 h-3.5 sm:w-4 sm:h-4" style={{ color: item.spriteColor }} />}
                          </div>
                          <div>
                            <p className="text-[10px] sm:text-xs font-semibold">{item.name}</p>
                            <p className="text-[8px] sm:text-[10px] text-slate-400 capitalize font-mono">{item.type}</p>
                          </div>
                        </div>
                        <div className="px-1.5 py-0.5 bg-slate-800 border border-slate-700 text-slate-300 rounded font-mono text-[9px] sm:text-[10px] font-bold">
                          x{item.quantity}
                        </div>
                      </button>
                    );
              ...
              <div className="w-full md:w-64 bg-slate-900/60 p-3 sm:p-5 flex flex-col justify-between space-y-2 sm:space-y-4">
                      ...
                      return (
                        <>
                          <div className="flex flex-col items-center text-center space-y-1.5 sm:space-y-3">
                            <div
                              className="w-10 h-10 sm:w-16 sm:h-16 rounded-2xl flex items-center justify-center border-2 animate-pulse"
                              style={{
                                backgroundColor: `${selectedItem.spriteColor}15`,
                                borderColor: `${selectedItem.spriteColor}40`,
                              }}
                            >
                              {selectedItem.type === 'weapon' && <Sword className="w-5 h-5 sm:w-8 sm:h-8" style={{ color: selectedItem.spriteColor }} />}
                              {selectedItem.type === 'shield' && <Shield className="w-5 h-5 sm:w-8 sm:h-8" style={{ color: selectedItem.spriteColor }} />}
                              {selectedItem.type === 'potion' && <Heart className="w-5 h-5 sm:w-8 sm:h-8" style={{ color: selectedItem.spriteColor }} />}
                              {selectedItem.type === 'key_item' && (
                                selectedItem.name === 'Gema Estelar'
                                  ? <Star className="w-5 h-5 sm:w-8 sm:h-8" style={{ color: selectedItem.spriteColor }} />
                                  : <Key className="w-5 h-5 sm:w-8 sm:h-8" style={{ color: selectedItem.spriteColor }} />
                              )}
                              {selectedItem.type === 'material' && <Sparkles className="w-5 h-5 sm:w-8 sm:h-8" style={{ color: selectedItem.spriteColor }} />}
                            </div>
                            <div>
                              <h4 className="text-xs sm:text-sm font-bold text-slate-100">{selectedItem.name}</h4>
                              <span className="inline-block mt-0.5 px-1.5 py-0.5 bg-slate-800 border border-slate-700/60 rounded text-[8px] font-mono font-bold uppercase tracking-wider text-amber-500">
                                {selectedItem.type}
                              </span>
                            </div>
                          </div>
